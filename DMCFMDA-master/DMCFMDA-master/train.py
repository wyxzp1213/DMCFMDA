...
def train(data,args):
    ...
    for i in range(kfolds):
        ...
        for epoch in epochs:
            ...
            md_copy[:, 1] = md_copy[:, 1] + args.microbe
            md_graph = dgl.graph(
                (np.concatenate((md_copy[:, 0], md_copy[:, 1])), np.concatenate((md_copy[:, 1], md_copy[:, 0]))),
                num_nodes=args.microbe + args.disease_number)
            microbe_th=th.Tensor(microbe)
            disease_th=th.Tensor(disease)

            #train_samples_th = th.Tensor(data['train_samples']).float()
            train_samples_th = th.Tensor(a).float()
            train_score,m1,m2,d1,d2 = model(mm_graph, dd_graph, md_graph, microbe_th, disease_th, a,epoch,Issne=True,)
            ...
            scoree, _, _, _, _ = model(mm_graph, dd_graph, md_graph, microbe_th, disease_th, b,epoch,Issne=False,)
            scoree = scoree.cpu()
        ...
        model.eval()
        scoree,_,_,_,_ = model(mm_graph, dd_graph, md_graph, microbe_th, disease_th, b,epoch=2,Issne=False)
        scoree = scoree.cpu()
        scoree = scoree.detach().numpy()
