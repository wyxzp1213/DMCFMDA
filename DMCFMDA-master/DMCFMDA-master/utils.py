...
def predict_case(data, args):
    data['m_d_matrix'] = make_adj(data['m_d'], (args.microbe, args.disease_number))
...
def data_processing(data, args):
    md_matrix = make_adj(data['md'], (args.microbe, args.disease_number))
    one_index = []
    zero_index = []
    ...
    md_matrix = make_adj(md, (args.microbe, args.disease_number))
    md_matrix = md_matrix.numpy()

    gm = get_gaussian(md_matrix)
    gd = get_gaussian(md_matrix.transpose())
    ms = np.loadtxt(args.data_dir + 'km.txt', dtype=float, delimiter=',')
    ds = np.loadtxt(args.data_dir + 'ID.txt', dtype=float)
    data['ms'] = ms
    data['ds'] = ds
...
def get_data(args):
    ...
    mf = np.loadtxt(args.data_dir + 'km.txt', dtype=float, delimiter=',')    #M-D

    dss = np.loadtxt(args.data_dir + 'ID.txt', dtype=float)    #M-D

    data['microbe'] = int(mf.shape[0])
    data['disease_number'] = int(dss.shape[0])
    data['mf'] = mf
    data['dss'] = dss
    data['md'] = np.loadtxt(args.data_dir + 'knowinteraction.txt', dtype=int) - 1   #M-D
